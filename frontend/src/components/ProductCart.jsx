...
export default function ProductCart({ product }) {
  ...
  return (
    <div className="product-box flex row bg-slate-200 p-5 items-center w-[90%] my-2 containerCardsCart">
      <img className="w-[10rem] h-[10rem] object-cover" src={product?.photo} alt="..." />
      <div className="flex flex-col m-4 grow container1">
        <div className="product">
        <div><h4 className="font-bold">{product?.name}</h4></div>
        <button className="trash" onClick={(e) => handleRemoveAll(product?._id, e)}>
          <DeleteIcon/>
        </button>
        </div>
